...
`,
	...
	cobraCmd.Flags().Int64VarP(&o.ExecutionCount, "count", "c", 1, "Defines how many times every test should be executed. ExecitionCount and MaxRetries flags are mutually exclusive.")
	cobraCmd.Flags().Int64VarP(&o.MaxRetries, "max-retries", "", 0, "Defines how many times a given test is retried in case of its failure. A suite is marked as a Succeeded even if some test failed and then finally succeeded. The default value is 0, which means that there are no retries of a given test.")
