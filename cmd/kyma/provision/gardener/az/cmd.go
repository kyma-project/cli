...
Create a service account with the ` + "`contributor`" + ` role. Use service account details to create a Secret and import it in Gardener.`,
	...
	cmd.Flags().StringVarP(
		...
		"Path to the kubeconfig file of the Gardener service account for Azure. (required)",
	)
	cmd.Flags().StringVarP(
		&o.Secret, "secret", "s", "", "Name of the Gardener secret used to access Azure. (required)",
	)
	cmd.Flags().StringVarP(
