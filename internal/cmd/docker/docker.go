...
import (
	...
	"github.com/kyma-project/cli.v3/internal/cmd/step"
...
func (w *dockerWrapper) PullImageAndStartContainer(ctx context.Context, opts ContainerRunOpts) (string, error) {
	...
	err = w.Docker.ContainerStart(ctx, body.ID, container.StartOptions{})
	if err != nil {
		return "", err
	}
...
func (w *dockerWrapper) ContainerFollowRun(ctx context.Context, containerID string, forwardOutput bool) error {
	buf, err := w.Docker.ContainerAttach(ctx, containerID, container.AttachOptions{
		Stdout: true,
		Stderr: true,
		Stream: true,
	})
