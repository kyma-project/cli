...
```
  ...
  -n, --name string                    Name of the cluster to provision. (required)
      ...
      --nodes int                      Number of cluster nodes. This will be set as scaler-max value(maximum autoscale value for cluster). (default 3)
  ...
  -r, --region string                  Region of the cluster. (default "europe-west3")
      --scaler-max nodes               Maximum autoscale value of the cluster. The value of nodes overrides the scaler-max value. (default 3)
