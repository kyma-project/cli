...
| **ifNil**         | bool, string, int, map, path | Conditionally returns one of two values based on whether a flag is nil. Takes three arguments: `ifNil "valueIfNotNil" "valueIfNil"`. If the flag is not nil, returns `valueIfNotNil`. If the flag is nil, returns `valueIfNil` ( both of which can be a template that gets processed as seen in example). | `annotations: ifNil "{'sidecar.istio.io/inject':'${{.flags.istioinjection.value}}'}" "" }}`                     |
