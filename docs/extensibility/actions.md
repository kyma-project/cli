...
| **wasUsed**       | bool, string, int, map, path | Checks if the flag was used (*is nil*) and returns appropriate value for it <br /> Supports **all types** of flags using two arguments in format `wasUsed "ValueIfNotNil" "ValueIfNil"` <br/> And for **boolean** using three in format `wasUsed "ValueIfTrue" "ValueIfFalse" "ValueIfNil"` | `annotations: ${{ .flags.istioinjection.value \| wasUsed "{'sidecar.istio.io/inject':'true'}" "{'sidecar.istio.io/inject':'false'}" "" }}` |
