...
| **ifNil**         | bool, string, int, map, path | Checks if the flag was used (*is nil*) and returns appropriate value for it <br /> Supports **all types** of flags using two arguments in format `ifNil "ValueIfNotNil" "ValueIfNil"` <br/> And for **boolean** using three in format `ifNil "ValueIfTrue" "ValueIfFalse" "ValueIfNil"` | `annotations: ${{ .flags.istioinjection.value \| ifNil "{'sidecar.istio.io/inject':'true'}" "{'sidecar.istio.io/inject':'false'}" "" }}` |
